    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8 mb-8">
            <div class="flex items-center justify-between mb-4">
                <a href="{{ route('quiz.index') }}" class="text-green-600 hover:text-green-700 font-semibold">
                    ← Kembali
                </a>
                <div class="text-gray-600">
                    Pertanyaan <span class="font-bold text-green-700">{{ $questions->count() }}</span>
                </div>
            </div>
            <h1 class="text-3xl font-bold text-green-700">
                {{ $type === 'self_efficacy' ? 'Self Efficacy' : 'Family Social Factor' }}
            </h1>
            <p class="text-gray-600 mt-2">
                @if($type === 'self_efficacy')
                    Kuisioner ini terdiri dari 2 bagian: Self-Efficacy dan Well-Being. Jawab semua pertanyaan dengan jujur sesuai dengan kondisi Anda.
                @else
        ...
        <form action="{{ route('quiz.submit', $type) }}" method="POST" class="space-y-8">
            ...
            <div class="bg-white rounded-xl shadow-lg p-6">
                <button type="submit" class="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-4 rounded-lg shadow-lg transition transform hover:scale-105">
                    Submit
                </button>
            </div>
        </form>
